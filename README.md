...
InkFlow は本レンダリング時に、既定位置の前後12%の範囲でいちばん広い余白帯（ほぼ白い帯）を自動で探し、見つかればそこへ分割線を寄せる。適した余白が見つからない場合（写真や図版でページが埋まっている場合など）は既定位置のまま出力するので、誤検出で変な位置に飛ぶことはない。

自動検出が誤る・もっと寄せたい場合は、右パネルの「分割位置の微調整」で `[－] [自動] [＋]` を使って左右・上下それぞれ手動で調整できる（1クリック1%、最大±20%）。手動で値を入れた軸は自動検出より優先される。「自動」ボタンで自動検出に戻せる。プレビューの分割枠は実際に使われる位置（自動検出または手動値）をそのまま反映する。
