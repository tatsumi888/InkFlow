...
1. 記事PDFをドラッグ＆ドロップで追加し、読む順に並べる
2. 既定レイアウト（二段組4分割＋俯瞰）が全ページに入っているので、**例外ページだけ**直す
3. ［編集］→［本の設定］で雑誌名と号を入れる（表紙が自動生成される）
4. ［EPUBを出力］
...
| `O` | ページ全体（俯瞰）の有無を切り替え |
| `R` | 縦横入替（なし → 右90° → 左90°） |
| `Ctrl+O` / `Ctrl+S` / `Ctrl+E` | PDF追加 / 保存 / EPUB出力 |
...
主なオプション: `--layout` `--no-overview` `--rotate` `--overlap` `--no-trim` `--device` `--format` `--gray-levels` `--gamma` `--cover`
...
**縦長のコマでは逆効果**なので、自動判定はせずページごとに選ぶ。回転はそのページの出力すべて（俯瞰ページも含む）に適用される。1ページ分を読む間に端末を持ち替えずに済ませるため。
