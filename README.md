...
原稿1ページを「ページ全体（俯瞰）＋分割コマ」の複数ページに展開し、月刊誌1号分をまとめた1冊として出力する。記事ごとにしおりが付き、俯瞰ページには原稿ページ番号のしおりが記事しおりの下に階層化されて付く。
...
| 右 | このページの分割レイアウト、俯瞰ページの有無、分割位置の微調整、ショートカットプリセット、まとめて適用、EPUB出力 |
...
`.inkflow.json` をウィンドウにドラッグ＆ドロップすると、そのプロジェクトを開ける。誤操作を防ぐため、内容（JSON構造・参照PDFの存在）を検証したうえで、タイトルや記事数を示して開いてよいか確認する。不整合があれば開かずにエラーを表示する。PDFと同時にドロップした場合はプロジェクトを開く方が優先される。
...
| `A` `S` `D` `F` `G` | ショートカットプリセットを適用（右パネルに常時表示） |
| `Z` `X` `C` `V` `B` | いまのページの分割設定をショートカットプリセットへ保存（同じ列がA↔Z等の対） |
| `Ctrl+N` / `Ctrl+O` / `Ctrl+S` / `Ctrl+E` | 新規プロジェクト / PDF追加 / 保存 / EPUB出力 |
| `F1` | キー操作の一覧 |

### ショートカットプリセット

例外ページでよく使う分割設定（レイアウト・俯瞰の有無・縦横入替・分割位置の微調整をすべて含む）を、キー一発で呼び出せるようにする機能。右手をマウスに置いたまま左手だけで操作できるよう、キーはすべてQWERTY配列の左手側（ホームロー `A S D F G` と、その真下の `Z X C V B`）から選んである。列を揃えているので対応を覚えやすい（`A`で適用する内容は`Z`で上書き保存する、`S`と`X`も同様）。

- `Z`〜`B`：いま見ているページの分割設定を、対応するスロットへ上書き保存
- `A`〜`G`：そのスロットに保存済みの設定を、いま見ているページへ適用（未設定なら何もしない）

右パネルの「ショートカットプリセット」に5スロット分の内容を常時表示するので、何が割り当たっているか一目で分かる。割り当ては `%APPDATA%\InkFlow\config.json` に保存され、プロジェクトをまたいで（アプリを再起動しても）引き継がれる。
...
主なオプション: `--layout` `--no-overview` `--rotate` `--rotate-overview` `--overlap` `--no-trim` `--column-bias` `--row-bias` `--device` `--format` `--gray-levels` `--gamma` `--cover`
...
`quad_2col` などの2段組レイアウトは、分割線を版面のちょうど中央（50%）に置くのが既定の考え方だが、実際の誌面は左右の段幅がわずかに非対称なことが多い（サイドの飾り罫・ノンブル・タブ状の飾りなど）。50%固定のまま切ると、本文の途中で文字が切断されてしまうことがある。

これに対して、既定位置の前後12%の範囲でいちばん広い余白帯（ほぼ白い帯）を探し、見つかればそこへ分割線を寄せる**自動検出**を用意している。ただし版面によっては自動検出が的外れな余白を拾って過大に寄せてしまうことがあるため、**既定では無効**にしてあり、既定位置（オフセット無し）に固定して出力する。

自動検出を試したいページだけ、右パネルの「分割位置の微調整」で `[自動]` を押すと有効になる（適した余白が見つからなければ既定位置のまま。誤検出で変な位置に飛ぶことはない）。`[－] [＋]` で左右・上下それぞれ手動で微調整もできる（1クリック1%、最大±20%）。`[既定]` を押すといつでも既定位置固定に戻せる。プレビューの分割枠は実際に使われる位置（既定位置・自動検出・手動値のいずれか）をそのまま反映する。

CLIから一括で指定する場合は `--column-bias`（左右）/ `--row-bias`（上下）を使う（`-0.2`〜`0.2`、プロジェクト全ページに適用）。
...
- 目次は2階層。記事しおり（記事の先頭ページ）の下に、俯瞰ページごとの子しおり（タイトルは原稿ページ番号、例「3ページ目」）がぶら下がる
