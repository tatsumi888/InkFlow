...
「号ごと」ではなく「アプリの使い方」の設定（ショートカットプリセットの割り当てなど）はプロジェクトJSONには含めず、`inkflow/gui/shortcut_presets.py` が `%APPDATA%\InkFlow\config.json` へ別途永続化する。プロジェクトを切り替えても、アプリを再起動しても引き継がれる。
